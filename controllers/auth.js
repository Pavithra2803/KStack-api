...
exports.register = (req, res) => {
    ...
    User.findOne({email}).exec((err,user) => {
        ...
        const token = jwt.sign({name, email, password, categories}, process.env.JWT_ACCOUNT_ACTIVATION, {
            expiresIn: '10m' //token becomes invalid after 10mins
        });
    ...
    sendEmailOnRegister
        .then(data => {
            ...
            res.json({
                message: `Email has been sent to ${email}, Follow the instrutions to complete your registration`
            })
        })
        .catch(error => {
...
exports.login = (req, res) => {
    ...
    User.findOne({ email }).exec((err, user) => {
        ...
        //Grabbing the user from db and sending to client
        return res.json({
            token,
...
//If we send any valid token this method will give us req.user ie. id 
exports.requireSignin = expressJwt({ secret: process.env.JWT_SECRET }); // req.user ie.user id will be available here

//Anywhere in the application we will be able to access currently logged in user info by accessing req.profile
...
//Users can delete only the links created by them
